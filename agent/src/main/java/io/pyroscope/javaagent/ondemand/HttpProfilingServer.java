    ...
    public void start() throws IOException {
        ...
        server.createContext("/profile/start", new HttpHandler() {
            ...
            public void handle(HttpExchange exchange) throws IOException {
                ...
                try {
                    // Parse request body for duration parameter
                    String body = new String(exchange.getRequestBody().readAllBytes());
                    JsonObject request = body.isEmpty() ? new JsonObject() : gson.fromJson(body, JsonObject.class);
                    long duration = request.has("duration") ?
                        request.get("duration").getAsLong() : 30; // Default 30 seconds

                    OnDemandProfilingController.ProfilingResult result =
                        controller.startProfiling(duration);
